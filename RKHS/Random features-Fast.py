...
#Reshaping datset necessary for the RFF method
X = np.reshape(X1[:Datasize],(Datasize,1))
Y = np.reshape(Y1[:Datasize],(Datasize,1))

#Declare time-taken variables
Trkhs = 0
Trff = 0
Tprob = 0
...
for i in range(numTP + 1):
    ...
    #ideal/ reference curve calculation
    r = tp**2
    sq.append(r)        
    tp += interval
